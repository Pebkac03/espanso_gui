...
import 'espanso_selection_menu_item_widget.dart';
import 'espanso_selection_menu_service.dart';

typedef EspansoSelectionMenuItemHandler = void Function(
  EditorState editorState,
  EspansoSelectionMenuService menuService,
  BuildContext context,
);

/// Selection Menu Item
class EspansoSelectionMenuItem {
  EspansoSelectionMenuItem({
    required String Function() getName,
    required this.icon,
    required this.keywords,
    required EspansoSelectionMenuItemHandler handler,
  }) : _getName = getName {
    this.handler = (editorState, menuService, context) {
      if (deleteAt) {
        _deleteAt(editorState);
      }
  ...
  final Widget Function(
    EditorState editorState,
    bool onSelected,
    EspansoSelectionMenuStyle style,
  ) icon;
  ...
  late final EspansoSelectionMenuItemHandler handler;

  VoidCallback? onSelected;

  bool deleteAt = true;

  void _deleteAt(EditorState editorState) {
    ...
    final lastAtIndex = delta.toPlainText().substring(0, end).lastIndexOf('/');
  ...
  /// has been inserted.
  factory EspansoSelectionMenuItem.node({
    required String Function() getName,
    ...
    return EspansoSelectionMenuItem(
      getName: getName,
...
class EspansoSelectionMenuStyle {
  const EspansoSelectionMenuStyle({
    required this.espansoSelectionMenuBackgroundColor,
    required this.espansoSelectionMenuItemTextColor,
    required this.espansoSelectionMenuItemIconColor,
    required this.espansoSelectionMenuItemSelectedTextColor,
    required this.espansoSelectionMenuItemSelectedIconColor,
    required this.espansoSelectionMenuItemSelectedColor,
  });

  static const light = EspansoSelectionMenuStyle(
    espansoSelectionMenuBackgroundColor: Color(0xFFFFFFFF),
    espansoSelectionMenuItemTextColor: Color(0xFF333333),
    espansoSelectionMenuItemIconColor: Color(0xFF333333),
    espansoSelectionMenuItemSelectedTextColor: Color.fromARGB(255, 56, 91, 247),
    espansoSelectionMenuItemSelectedIconColor: Color.fromARGB(255, 56, 91, 247),
    espansoSelectionMenuItemSelectedColor: Color(0xFFE0F8FF),
  );

  static const dark = EspansoSelectionMenuStyle(
    espansoSelectionMenuBackgroundColor: Color(0xFF282E3A),
    espansoSelectionMenuItemTextColor: Color(0xFFBBC3CD),
    espansoSelectionMenuItemIconColor: Color(0xFFBBC3CD),
    espansoSelectionMenuItemSelectedTextColor: Color(0xFF131720),
    espansoSelectionMenuItemSelectedIconColor: Color(0xFF131720),
    espansoSelectionMenuItemSelectedColor: Color(0xFF00BCF0),
  );

  final Color espansoSelectionMenuBackgroundColor;
  final Color espansoSelectionMenuItemTextColor;
  final Color espansoSelectionMenuItemIconColor;
  final Color espansoSelectionMenuItemSelectedTextColor;
  final Color espansoSelectionMenuItemSelectedIconColor;
  final Color espansoSelectionMenuItemSelectedColor;
}

class EspansoSelectionMenuWidget extends StatefulWidget {
  const EspansoSelectionMenuWidget({
    super.key,
    required this.items,
    ...
    required this.deleteAtByDefault,
  });

  final List<EspansoSelectionMenuItem> items;
  ...
  final bool deleteAtByDefault;

  @override
  State<EspansoSelectionMenuWidget> createState() =>
      _espansoSelectionMenuWidgetState();
}

class _espansoSelectionMenuWidgetState
    extends State<EspansoSelectionMenuWidget> {
  final _focusNode = FocusNode(debugLabel: 'popup_list_widget');

  int _selectedIndex = 0;
  List<EspansoSelectionMenuItem> _showingItems = [];
  ...
  set keyword(String newKeyword) {
    ...
    if (keyword.length >= maxKeywordLength + 2 &&
        !(widget.deleteAtByDefault && _searchCounter < 2)) {
    ...
    return Focus(
      ...
      child: DecoratedBox(
        decoration: BoxDecoration(
          color: widget
              .espansoSelectionMenuStyle.espansoSelectionMenuBackgroundColor,
          boxShadow: [
            BoxShadow(
              blurRadius: 5,
              spreadRadius: 1,
  ...
  Widget _buildResultsWidget(
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      itemWidgets.add(
        EspansoSelectionMenuItemWidget(
          item: items[i],
          isSelected: selectedIndex == i,
          editorState: widget.editorState,
          menuService: widget.menuService,
          espansoSelectionMenuStyle: widget.espansoSelectionMenuStyle,
        ),
      );
    }
  ...
  KeyEventResult _onKeyEvent(FocusNode node, KeyEvent event) {
    Log.keyboard.debug('at command, on key $event');
